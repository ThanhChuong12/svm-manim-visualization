...
class ScoreCombinationScene(MovingCameraScene):
    ...
    def _create_scanner_panel(self, icon_factory, title_text, accent_color, position):
        ...
        laser = Rectangle(
            width=panel_w * 0.75, height=0.04, fill_color=LASER_CYAN, fill_opacity=0.8, stroke_width=0
        ).move_to(panel_bg.get_top() + DOWN * 0.3)
        return panel_bg, icon, title, laser

    def _phase1_vectorization(self, axes, x_label, y_label):
        """Transform independent biometric scores into a 2D vector representation."""
        face_bg, face_icon, face_title, face_laser = self._create_scanner_panel(
            make_genuine_icon, "Face Recognition", CLASS_B_COLOR, LEFT * 3.2 + UP * 2.4
        )
        fp_bg, fp_icon, fp_title, fp_laser = self._create_scanner_panel(
            make_fingerprint_icon, "Fingerprint Scan", FP_COLOR, RIGHT * 3.2 + UP * 2.4
        )
        
        self.play(
            FadeIn(face_bg), FadeIn(fp_bg), FadeIn(face_title), FadeIn(fp_title),
            GrowFromCenter(face_icon), GrowFromCenter(fp_icon), run_time=1.0
        )
        ...
        self.play(
            ...
            FadeIn(face_laser), FadeIn(fp_laser), run_time=1.2, rate_func=linear
        )
        ...
        score_matrix = Matrix(
            [["0.85"], ["0.92"]], left_bracket="[", right_bracket="]", 
        ...
        self.play(
            FadeOut(face_bg), FadeOut(fp_bg), FadeOut(face_icon), FadeOut(fp_icon),
        ...
        self.play(
            ...
            flying_dot.animate.move_to(target_pos), Create(guide_x), Create(guide_y),
        ...
        self.play(
            FadeOut(guide_x), FadeOut(guide_y), FadeOut(coord_label),
            FadeOut(sample_dot), FadeOut(vector_group), run_time=0.8
        )
        self.wait(0.2)
    ...
    def _phase2_math_detour(self):
        ...
        Teach SVM concept using abstract data in a standard grid layout.
        Demonstrate the iterative optimization to find the maximum margin.
        """
        # 1. Initialize mathematical grid and toy dataset
        blue_points = np.array([
            [-3.0, -1.0], [-2.5, -2.0], [-1.5, -2.5], [-2.0, -0.5], 
            [-1.0, -1.5], [-0.5, -3.0], [-4.0, -1.5], [-3.5, -3.0],
            [-0.5, -0.5] # Support Vector
        ])
        red_points = np.array([
            [1.0, 3.0], [2.0, 2.5], [1.5, 4.0], [2.5, 3.5],
            [3.5, 2.0], [3.0, 1.5], [4.0, 3.0], [3.0, 4.0],
            [0.5, 1.5],  # Support Vector
            [1.5, 0.5]   # Support Vector
        ])
        ...
        dots = VGroup(*[
            Dot(point=plane.c2p(pt[0], pt[1]), color=CLASS_A_COLOR if label == 0 else CLASS_B_COLOR, radius=0.08) 
        ...
        # 2. Compute optimal SVM using scikit-learn
        svm = SVC(kernel="linear", C=1000).fit(X, y)
        w_opt, b_opt = svm.coef_[0], svm.intercept_[0]
        ...
        # 3. Setup ValueTrackers for searching animation
        angle_tracker = ValueTracker(opt_angle + PI / 4) # Start offset by 45 degrees
        margin_tracker = ValueTracker(0.0)
        ...
        # 4. Iterate through test angles to simulate learning
        test_angles = [
    ...
    def _phase3_application(self, axes):
        """Restore biometric context and apply linear SVM to rescue edge cases."""
        ...
        rescue_arrow_g = Arrow(
            noisy_start_pos + UP * 0.20, noisy_target_center + DOWN * 0.40, 
            color=GENUINE_COLOR, stroke_width=2, stroke_opacity=0.6, buff=0.1, max_tip_length_to_length_ratio=0.12
        )
        ...
        rescue_arrow_s = Arrow(
            spoof_mid_center + DOWN * 0.2, spoof_final_center + UP * 0.45, 
            color=IMPOSTOR_COLOR, stroke_width=2, stroke_opacity=0.6, buff=0.1, max_tip_length_to_length_ratio=0.12
        )
        ...
        # Expand crowd mapping from 1D to 2D
        ...
        # Snap Final SVM - strict clipping inside axes bounds
        ...
        w_final, b_final = clf.coef_[0].copy(), clf.intercept_[0]
        ...
        # Polygon Clipping: collect margin edges + corners
        band_pts_data = []
        band_pts_data.extend(mp_pts)
        band_pts_data.extend(mn_pts[::-1]) # Reverse negative margin to form closed loop
        ...
        # Sort points clockwise to prevent twisted polygons
    ...
    def _phase4_xor_trap(self, axes, genuine_cloud, impostor_cloud, hyperplane):
        """Introduce the spoofing attack that defeats the linear model via XOR topology."""
        # 1. Warning System Flash
        ...
        tl_cap_group = VGroup(tl_caption_bg, tl_caption).next_to(spoof_tl, UP, buff=0.25)
